 ...
 * このエンドポイントが返す形。
 *
 * Minecraft サーバーの状態は mcsrvstat.us から来たものをそのまま widen して
 * 返しており、項目はサーバーの種類によって変わる。こちらで必ず足す分だけを
 * 名前で書き、残りは索引で受ける。
 */
...
export const GET: APIRoute = async ({ request }) => {
	...
	let statusData: ServerStatus = { online: false };
	...
	try {
		...
		if (pingHistory.length > 0) {
			...
			if (pingHistory.length > 1) {
				...
				const normalizedY =
					height - (pingHistory[0].avg / (maxPing * 1.2)) * height;
	...
	try {
		if (type === "minecraft") {
			...
			const res = await fetch(
				`https://api.mcsrvstat.us/${apiEndpoint}/${fetchAddress}`,
			);
			if (res.ok) {
			...
			if (!statusData.online && !isBedrock) {
				const fallbackRes = await fetch(
					`https://api.mcsrvstat.us/2/${fetchAddress}`,
				);
				if (fallbackRes.ok) {
					const fbData = (await fallbackRes.json()) as Record<string, unknown>;
					if (fbData.online) {
						statusData = {
							...fbData,
							online: true,
	...
	if (statusData.availability === undefined)
		statusData.availability = availability;
	if (statusData.avgPing === undefined) statusData.avgPing = avgPing;
	if (statusData.sparklinePath === undefined)
		statusData.sparklinePath = sparklinePath;
