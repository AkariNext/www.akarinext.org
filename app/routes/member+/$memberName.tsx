                    ...
                    <div className="bg-white py-10">
                        <div className="flex content-between akari-container flex-col md:flex-row">
                            <div className="flex items-center gap-4 w-full mb-4 md:mb-0">
                                <Avatar src={res.member.avatar} alt={res.member.name} size="lg" />
                                <div>
                                ...
                                <Await resolve={res.posts}>
                                    {posts => (
                                        <>
                                            {posts.map(post => (
                                                <div key={post.slug}>
                                                    <ArticleCardWithLink title={post.title} emoji={post.emoji} classes={{ root: "h-full" }} slug={post.slug} dateDisplay={post.dateDisplay} />
                                                </div>
                                            ))}
                                        </>
                                    )}
