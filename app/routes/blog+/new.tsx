...
export default function EditArticle() {
    ...
    return (
        <div className='akari-container grid sm:grid-cols-1 md:grid-cols-12 py-10 gap-4 grid-flow-dense'>
            <div className='col-span-10'>
                {<div ref={editor} className={cn({'hidden': isPreview === true})}></div>}
                {previewFetcher.state === 'loading' && <div className='mdx m-0'>プレビューを生成中です...</div>}
                {previewFetcher.state !== 'loading' && isPreview && previewContent && (
                    <div
                        className="mdx m-0"
            ...
            <div className='relative col-span-2 flex gap-2 h-min bg-white border rounded-full w-min px-2 py-1 items-center'>
                <div className={cn('absolute h-10 w-10 px-2 py-1 rounded-full z-10 transition-all duration-300', isPreview ?"right-2" : "right-14", isPreview ? "bg-blue-400" : "bg-gray-200")} style={{content: ""}}></div>
                <button onClick={onEditor} className={cn('flex justify-center items-center h-10 w-10 rounded-full p-2 bg-transparent z-20 text-gray-400')}>
                    <IconPencil stroke={2} />
                </button>
                <button onClick={onPreview} className={cn('flex justify-center items-center h-10 w-10 rounded-full p-2 bg-transparent z-20 text-gray-200')}>
