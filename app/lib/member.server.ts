...
const memberRoleEnum = z.enum(['mod', 'dev', 'designer']);
...
function convertDecoratedRoles(
	roles: PickType<z.infer<typeof memberSchema>, 'roles'>,
): string[] {
...
function validateMember(member: any[]): TMember[] {
	...
	for (const m of member) {
		...
		members.push({
			...result.data,
			roles: convertDecoratedRoles(result.data.roles),
		});
	}
